   ...
   public:
    ...
    Polygon normalize_poly(const Polygon& ref, const Polygon& p)
    {
        ...
        for (size_t start = 0; start < 4; start++) {
            size_t j = start;
            std::int64_t d = (sqr(ref.poly[(0) % 4].X - p.poly[(j + 0) % 4].X) +
                              sqr(ref.poly[(0) % 4].Y - p.poly[(j + 0) % 4].Y) +
                              sqr(ref.poly[(1) % 4].X - p.poly[(j + 1) % 4].X) +
                              sqr(ref.poly[(1) % 4].Y - p.poly[(j + 1) % 4].Y) +
                              sqr(ref.poly[(2) % 4].X - p.poly[(j + 2) % 4].X) +
                              sqr(ref.poly[(2) % 4].Y - p.poly[(j + 2) % 4].Y) +
                              sqr(ref.poly[(3) % 4].X - p.poly[(j + 3) % 4].X) +
                              sqr(ref.poly[(3) % 4].Y - p.poly[(j + 3) % 4].Y));
            if (d < min_d) {
                min_d = d;
                best_start = start;
                best_order = 0;
            ...
            d = (sqr(ref.poly[(0) % 4].X - p.poly[(j + 3) % 4].X) +
                 sqr(ref.poly[(0) % 4].Y - p.poly[(j + 3) % 4].Y) +
                 sqr(ref.poly[(1) % 4].X - p.poly[(j + 2) % 4].X) +
                 sqr(ref.poly[(1) % 4].Y - p.poly[(j + 2) % 4].Y) +
                 sqr(ref.poly[(2) % 4].X - p.poly[(j + 1) % 4].X) +
                 sqr(ref.poly[(2) % 4].Y - p.poly[(j + 1) % 4].Y) +
                 sqr(ref.poly[(3) % 4].X - p.poly[(j + 0) % 4].X) +
                 sqr(ref.poly[(3) % 4].Y - p.poly[(j + 0) % 4].Y));
            if (d < min_d) {
                min_d = d;
                best_start = start;
                best_order = 1;
 ...
 */
std::vector<Polygon> standard_nms(std::vector<Polygon>& polys,
                                  float iou_threshold)
{
    size_t n = polys.size();
    if (n == 0)
...
std::vector<std::vector<float>> polys2floats(const std::vector<Polygon>& polys,
                                             int precision = 10000)
{
    ...
    for (size_t i = 0; i < polys.size(); i++) {
        ...
        ret.emplace_back(std::vector<float>{
            float(poly[0].X) / precision,
            float(poly[0].Y) / precision,
            float(poly[1].X) / precision,
            float(poly[1].Y) / precision,
            float(poly[2].X) / precision,
            float(poly[2].Y) / precision,
            float(poly[3].X) / precision,
            float(poly[3].Y) / precision,
            float(p.score),
        });
    }
...
std::vector<std::vector<float>> merge_quadrangle_n9(
    std::vector<std::vector<float>>& data, std::vector<float>& scores,
    float iou_threshold, int precision = 10000)
{
    using cInt = cl::cInt;
    ...
    for (size_t i = 0; i < data.size(); i++) {
        ...
        Polygon poly{
            {
                {cInt(p[0] * precision), cInt(p[1] * precision)},
                {cInt(p[2] * precision), cInt(p[3] * precision)},
                {cInt(p[4] * precision), cInt(p[5] * precision)},
                {cInt(p[6] * precision), cInt(p[7] * precision)},
            },
            scores[i],
        };
    ...
    return polys2floats(standard_nms(polys, iou_threshold), precision);
}
}  // namespace lanms
