              ...
              int width)
{
    for (auto& one_box : boxes) {
        std::vector<int> rect = trt_east::GetRect(one_box);
        ...
        for (int i = rect[0]; i <= rect[2]; i++) {
            for (int j = rect[1]; j <= rect[3]; j++) {
                sum_score = sum_score + f_score[j * width + i];
                //使用固定值0.1，过滤得分
                if (f_score[j * width + i] > 0.1) {
                    temp_score = temp_score + f_score[j * width + i];
                    num++;
                }
            }
        }
        avg_scores.push_back(
            sum_score / ((rect[2] - rect[0] + 1) * (rect[3] - rect[1] + 1)));
        final_scores.push_back(temp_score / num);
    }
}
